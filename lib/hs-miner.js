...
miner.hashShare = function hashShare(data, enc) {
  const hash = binding.hashShare(data);
  if (enc === 'hex')
    return hash.toString('hex');
  return hash;
}
...
miner.HAS_OPENCL = false;
miner.BACKEND = miner.getBackend();
miner.TARGET = Buffer.alloc(32, 0xff);
miner.HEADER_SIZE = 236;
miner.MINER_SIZE = 256;
miner.SHARE_SIZE = 128;
miner.NONCE_SIZE = 4;
miner.NONCE_START = 0;
miner.NONCE_END = 4;
miner.EXTRA_NONCE_SIZE = 24;
